...
### Function to import indicator description from the database schedule

readImportParams <- function (param_fname, update_mode) {
  
  #param_fname <- here("assets", "_DB", "0_database_params.xlsx")
  impplan <- readxl::read_excel(param_fname, sheet = "import", col_names = T, skip=1)
  parameters <- readxl::read_excel(param_fname, sheet = "scope", col_names = T, skip=1, n_max=1)
  locals <- readxl::read_excel(param_fname, sheet = "scope", col_names = T, skip=6)
...
### Functions to read and parse formulas from the indicator description file
...
### Functions to build all the objects of the dependency graph
...
# ---------- Function for plotting the interactive graph (ggiraph) -------------------
...
plot_indicator_graph_interactive <- function(dep_graph,
  ...
  # ---- edge attributes for the tooltips ----
