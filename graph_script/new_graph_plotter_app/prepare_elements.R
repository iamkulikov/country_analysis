...
.peers_codes_cache <- new.env(parent = emptyenv())

.peers_cache_key <- function(country_iso3c, peers_fname) {
  mtime <- if (file.exists(peers_fname)) {
    as.integer(file.info(peers_fname)$mtime)
  } else {
    NA_integer_
  }
  paste(normalize_iso3_strict(country_iso3c %||% ""), peers_fname, mtime, sep = "\x1f")
}
...
get_peers_cached <- function(country_iso3c, peers_fname) {
  ...
  if (!exists(key, envir = .peers_codes_cache, inherits = FALSE)) {
    assign(
      key,
      getPeersCodes(country_iso3c = country_iso3c, peers_fname = peers_fname),
      envir = .peers_codes_cache
    )
  }
  get(key, envir = .peers_codes_cache, inherits = FALSE)
}

clear_peers_cache <- function() {
  rm(list = ls(envir = .peers_codes_cache), envir = .peers_codes_cache)
  invisible(NULL)
}
