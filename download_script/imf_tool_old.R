imfTool <- function(code, database, freq, start, end) {
  ...
  end <- min(as.numeric(end), 2024) # костыль - заменить на запрос со временем
...
# adres <- "http://dataservices.imf.org/REST/SDMX_JSON.svc/CompactData/CPI/M..PCPI_IX.?startPeriod=2024&endPeriod=2025"
# imfTool(code = "CPI._T.IX", database = "CPI", freq = "A", start = "2019", end = "2022")
