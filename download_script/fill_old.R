...
fill <- function(fillplan, extdata_y, extdata_q, extdata_m, extdata_d) {
    ...
    for (i in 1:dim(fillplan)[1]) {
      ...
      if (oldfreq == newfreq & active == 1 & fillplan$formula[i] =="share" ) {
        ...
        a <- glue(
          "
          extdata_{oldfreq} <- extdata_{oldfreq} |>
            dplyr::group_by({oldfreq_long}) |>
            dplyr::mutate(
              world_total_tmp = if (any(country_id == '1W' & !is.na({oldcode}))) {{
                {oldcode}[country_id == '1W'][1]
              }} else {{
                sum({oldcode}[country_id != '1W'], na.rm = TRUE)
              }},
              {newcode} = dplyr::if_else(
                world_total_tmp == 0 | is.na(world_total_tmp),
                NA_real_,
                {oldcode} * 100 / world_total_tmp
              )
            ) |>
            dplyr::ungroup() |>
            dplyr::select(-world_total_tmp)
        "
        )
        eval(parse(text = a)); print(a)
  ...
  for (countryname_export in countries) {
    ...
    extdata_y |> filter(country==countryname_export) |> select(-c("country","country_id","year")) |>
      select(any_of(c('gdp_pc_usd_wb', 'gdp_pc_ppp_wb', 'gdp_growth', 'gdp_usd', 'gdp', 
                    ...
                    'ca_usd', 'ca_gdp', 'ex_gs_usd', 'imp_gs_usd', 'intres_usd', 'intrestoimp', 'niip_ex_ggcb_usd', 'niip_ex_ggcb_gdp', 
