...
countries <- c("Armenia", "Brazil", "Bulgaria", "Greece", "China", "India", "Iran, Islamic Rep", "Kazakhstan", "Kyrgyz Republic", "Romania",
...
if (is.null(dim(error_report)[1]) | is.na(dim(error_report)[1]) | (dim(error_report)[1] == 0)) {
      ...
      writeDatafilesCsv(datalist = FD, path = here("_country_analysis_scripts", "graph_script", "graph_plotter_app"))
