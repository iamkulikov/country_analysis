...
if (update_mode == 1) { impdata <- impdata %>% filter(update == 1) }
impdata <- impdata %>% filter(active == 1) 
...
###### Functions to check how successful is import?

checkImpStatus <- function(impdata, extdata_y, extdata_q, extdata_m, extdata_d, update_mode) {
  
  if (update_mode==1) {impdata <- impdata %>% filter(update == 1)}
  impdata <- impdata %>% filter(active == 1)
  extdata_d <- extdata_d %>% select(-date)
  impdata_frequencies <- impdata %>% select(source_frequency) %>% distinct() %>% unlist()
  empty_codes <- rbind(c('code', 'a'), c('code', 'a'))
  
  # include all empty columns from extdata
  for (i in impdata_frequencies) {
  ...
  empty_output <- impdata %>% inner_join(empty_codes, by=c("indicator_code", "source_frequency")) 
  ...
  nonexistent_output <- impdata %>% anti_join(existing_codes, by=c("indicator_code", "source_frequency"))
...
#checkImpStatus(impdata, extdata_y, extdata_q, extdata_m, extdata_d, update_mode = 1)
# tryImport <- function(impdata, extdata_y, extdata_q, extdata_m, extdata_d, update_mode) {}



###### Import cycle 

impdata_temp <- impdata
#impdata_temp

for (t in 1:10) {

    if ( dim(impdata_temp)[1] == 0 ) {break}
    
    
    ##### Drop data, which needs to be updated
    
      for (i in seq_along(impdata_temp$indicator)) { 
        
        eval(parse( text = paste0("if ('", impdata_temp$indicator_code[i], "' %in% names(extdata_", impdata_temp$source_frequency[i],")) { extdata_",
                                  impdata_temp$source_frequency[i], " <- extdata_",
                                  impdata_temp$source_frequency[i], " %>% select(-c(", impdata_temp$indicator_code[i],")) }") ))
        }

      
    ##### Import WDI
    
    wdi_impdata <- impdata_temp %>% filter(active==1, database_name=="WDI", retrieve_type=="API", source_frequency=="y")
    ...
    wgi_impdata <- impdata_temp %>% filter(active==1, database_name=="WGI", retrieve_type=="file", source_frequency=="y")
    ...
    un_impdata <- impdata_temp %>% filter(active==1, database_name=="HDR", retrieve_type=="API", source_frequency=="y")
    ...
    unctad_impdata <- impdata_temp %>% filter(active==1, source_name=="UNCTAD", retrieve_type=="file", source_frequency=="y")
    ...
    imfy_impdata <- impdata_temp %>% filter(active==1, source_name=="IMF", retrieve_type=="API",
                                       source_frequency=="y")
    ...
    imfq_impdata <- impdata_temp %>% filter(active==1, source_name=="IMF", retrieve_type=="API",
                                       source_frequency=="q")
    ...
    imfm_impdata <- impdata_temp %>% filter(active==1, source_name=="IMF", retrieve_type=="API",
                                       source_frequency=="m")
    ...
    ids_impdata <- impdata_temp %>% filter(active==1, database_name=="IDS", retrieve_type=="file", source_frequency=="y")
    ...
    bis_impdata <- impdata_temp %>% filter(active==1, source_name=="BIS", retrieve_type=="file", database_name=="Policy rates (monthly)")
    ...
    fm_impdata <- impdata_temp %>% filter(active==1, database_name=="FM", retrieve_type=="file", source_frequency=="y")
    ...
    ilo_impdata <- impdata_temp %>% filter(active==1, source_name=="ILO", retrieve_type=="API")
    ...
    covid_impdata <- impdata_temp %>% filter(active==1, source_name=="Ourworldindata", retrieve_type=="file", database_name== "COVID tracker")
    ...
    #####  Update impdata to retry failed import
    
    impdata_temp <- checkImpStatus(impdata, extdata_y, extdata_q, extdata_m, extdata_d, update_mode)
    #print(impdata_temp)
    #print(glue("Import cycle number {t} ended"))
