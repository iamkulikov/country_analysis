...
checkAvailability <- function(graphplan, dict, warn_unavailable = TRUE) {
  ...
  # ---- Row verdict + missing list (all graphplan rows) -----------------
  row_stats <- if (nrow(checked_long) == 0L) {
    tibble::tibble(
      .row_id = integer(),
      check_availability = integer(),
      missing = list()
    )
  } else {
    checked_long |>
  ...
  row_verdict <- gp |>
    ...
    dplyr::left_join(row_stats, by = dplyr::join_by(.row_id))

  if (!"check_availability" %in% names(row_verdict)) {
    row_verdict$check_availability <- rep(NA_integer_, nrow(row_verdict))
  }
  if (!"missing" %in% names(row_verdict)) {
    row_verdict$missing <- replicate(nrow(row_verdict), character(0), simplify = FALSE)
  }
  ...
  out <- gp |>
    dplyr::left_join(
      row_verdict |>
        dplyr::select(.data$.row_id, .data$check_availability, .data$missing),
      by = dplyr::join_by(.row_id)
    )

  if (!"check_availability" %in% names(out)) {
    out$check_availability <- rep(NA_integer_, nrow(out))
  }
  if (!"missing" %in% names(out)) {
